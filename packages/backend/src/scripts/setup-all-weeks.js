...
async function setupAllWeeks() {
  await connectPostgres();
  const pool = getPostgresPool();

  logger.info('=== STEP 1: Import Games ===\n');
  ...
  for (const game of SEASON_SCHEDULE) {
    try {
      ...
      if (existing.rows.length > 0) {
        skipped++;
        importedGameIds.push(existing.rows[0].id);
        logger.info(`⏭️  Week ${game.week}: ${game.home} vs ${game.away} (already exists)`);
      } else {
        ...
        imported++;
        importedGameIds.push(result.rows[0].id);
        logger.info(`✅ Week ${game.week}: ${game.home} vs ${game.away}`);
      }
    } catch (error) {
  ...
  logger.info(`\n✅ Import complete: ${imported} new, ${skipped} existing\n`);
  ...
  for (const gameId of importedGameIds) {
    try {
      const game = await pool.query(
        'SELECT id, week, home_team, away_team FROM games WHERE id = $1',
        [gameId]
      );
      ...
      logger.info(`  Week ${week}: Predicting ${home_team} vs ${away_team}...`);
  ...
  logger.info(`\n✅ Predictions complete: ${predictionsGenerated} success, ${predictionErrors} errors\n`);

  // Summary
  logger.info('=== SUMMARY ===');
  logger.info(`Games imported: ${imported}`);
  logger.info(`Games existing: ${skipped}`);
  logger.info(`Predictions generated: ${predictionsGenerated}`);
  logger.info(`Prediction errors: ${predictionErrors}`);

  await pool.end();
}

setupAllWeeks().catch(error => {
